...
serve(async (req) => {
  ...
  try {
    ...
    // First try getClaims for faster validation
    ...
    if (claimsError || !claimsData?.claims?.sub) {
      ...
      logStep("User authenticated via getUser", { userId: user.id, email: user.email });
      ...
      logStep("User authenticated via getClaims", { userId: claims.sub, email: claims.email });
    ...
    const { data: promoData } = await supabaseClient
      ...
      .select("promo_codes(grants_lifetime_access, is_active)")
      .eq("user_id", userId)
      .limit(1)
      .maybeSingle();

    if (promoData?.promo_codes?.grants_lifetime_access && promoData?.promo_codes?.is_active) {
      logStep("Promo code lifetime access", { userId });
